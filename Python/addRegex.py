...
def addRegex(modelo, path, output):
    ...
    for x in os.listdir(path):
        f = open(path + x)
        contents = f.read()
        smthProy = re.findall("(\S+(\s+[pP]roject(s)*))", contents) # para este regex comprobar que la primera letra de cada palabra de la lista sea mayuscula
        proyNumber = re.findall("([pP]roject(s)* (no. |No. |[Nn]umber )+([^\s]+))", contents)
        grantAgreement = re.findall("([gG]rant(s)* ([aA]greement )*([Nn]o(\.*) )*(number )*([a-zA-Z\-:/\.#0-9]+)*(['\"].+['\"])*)", contents)
        ...
        for i in proySmth:
            var = i[0].split(" ")[1]
            if var[0].isalpha() == True or var[0].isdigit() == True or var [0] == "'" or var[0] == "\"":
